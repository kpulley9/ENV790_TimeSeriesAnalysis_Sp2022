...
>>>>>>> cb07cb7157b7cdc6f2d6864eadd24dadc04e0f32
---
...
raw_inflow_data <- read.table(file="/Users/kristenpulley/Desktop/Time Series/ENV790_TimeSeriesAnalysis_Sp2022/Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
#main=paste is to name plots
#i=loop to run all pcf and acf w 1 code for 16 row
...
```{r}
#Create vector t=row numbers of ts
t <- c(1:nobs)
...
#Fit a linear trend to TS of iHP (by using a regression line)
#lm(dependent term~independent term (always time))
...
#subtract red aka trend aka regression line->(beta0+beta1*t)
...
ggplot(inflow_data, aes(x=my_date, y=inflow_data[,(1+iHP)])) +
            ...
            geom_smooth(aes(y=detrend_inflow_data),color="pink",method="lm") 

```
Note that blue line is our original series, red line is our trend, green line is our original series minus the trend or in other words the detrended series. And in pink is the trend line for the detrended series which has slope 0 meaning we were able to effectively eliminate the trend with a linear model.
...
#last month is the intercept
#bc trend is a consequence of month not time

#Then fit a linear model to the seasonal dummies
seas_means_model=lm(inflow_data[,(iHP+1)]~dummies)
summary(seas_means_model)
#if all variables are significant, then data is significant
...
#done to see if seasonal component is constant over time
...
#green = observations not tied to seaosonality
```
