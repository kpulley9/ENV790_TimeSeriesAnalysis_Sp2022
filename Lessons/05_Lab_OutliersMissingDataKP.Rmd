...
tswind<-ts(full_wind_data, frequency = 12)
decomp_wind=decompose(tswind[,3],"additive")
plot(decomp_wind)

#Ex2: Store the three component in separate vector
wind_rand<-decomp_wind$random
wind_seas<-decomp_wind$seasonal
wind_trend<-decomp_wind$trend

#Ex3: Create one data frame with all three vector and the Date columns from wind_data

full_decomp.df<-data.frame(Date=full_wind_data$Date,Random=wind_rand,Seasonal=wind_seas,Trend=wind_trend)

#Ex4: Inspect the data frame using head() and tail()

head(full_decomp.df,10)
tail(full_decomp.df,10)
...
The method that decompose runs the data set (subtract/add) to get the trends

## Coding challenge
```{r}

#Ex5: Try to reproduce the trend component from decompose().
#do the regression (lm) on the AWND and then plot against the column from decompose above

#Ex6: Try to reproduce the seasonal component from decompose().
#do the regression w dummies and subtract, plot against the seasonal column from above
